 ...
 * @param count 元素个数
 */
export function getRandomArrayElements<T>(array: Array<T>, count: number) {
 ...
 * @param list 目标数组
 */
export function arrayItemToHump<T>(list: Array<T>) {
  const result = [];
  for (let i = 0; i < list.length; i++) {
    const item = list[i];
    result.push(objectToHump(item));
  }
  return result;
}
 ...
 * @param target 目标对象
 */
export function objectToHump(target: any) {
