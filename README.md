...
```json
{
  ...
  "scripts": {
    ...
    "dev:test": "cross-env mode=test ts-node-dev --respawn ./src/index.ts",
    "dev:prod": "cross-env mode=prod ts-node-dev --respawn ./src/index.ts",
    "build": "node -e \"require(\"fs\").rmSync(\"dist\", { recursive: true, force: true })\" && tsc && node dist/index.js",
    "build:test": "node -e \"require(\"fs\").rmSync(\"dist\", { recursive: true, force: true })\" && tsc && cross-env mode=test node dist/index.js",
    "build:prod": "node -e \"require(\"fs\").rmSync(\"dist\", { recursive: true, force: true })\" && tsc && cross-env mode=prod node dist/index.js",
...
在以上json配置文件中`build`相关的重复代码如何优化使用？
